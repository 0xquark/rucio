...
# We don't copy the source code here - it will be mounted at runtime
COPY requirements requirements
COPY setup.py setup_rucio.py setup_rucio_client.py setup_webui.py setuputil.py ./
COPY .pep8 .pycodestyle pyproject.toml ./
...
# Pre-install Rucio dependencies but not Rucio itself
RUN pip install -e .[oracle,postgresql,mysql,kerberos,saml,dev] --no-deps && \
    python3 -m pip list
...
# Create directory for mounting source code
WORKDIR /opt/rucio
# No copying of source code here - it will be mounted at runtime

# Add entrypoint script to install Rucio from mounted source
COPY etc/docker/test/extra/entrypoint.sh /entrypoint.sh
